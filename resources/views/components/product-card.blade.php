    ...
    <a href="{{ route('products.show', $product->slug) }}" class="block">
        ...
        <div class="pt-4">
            <div class="flex items-start justify-between gap-4">
                <div><p class="text-[9px] uppercase tracking-[.18em] text-muted">{{ $product->category?->name ?? 'Colección JV' }}</p><h3 class="mt-1 font-sans text-sm font-medium leading-5">{{ $product->name }}</h3></div>
