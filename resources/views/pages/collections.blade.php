...
<section id="todas" class="section-pad !pt-16"><div class="shell">@if($products->isNotEmpty())<div class="grid grid-cols-2 gap-x-3 gap-y-12 md:gap-x-6 lg:grid-cols-4 lg:gap-y-16">@foreach($products as $product)<x-product-card :product="$product" />@endforeach</div>@else<x-empty-catalog-state />@endif</div></section>
<x-catalog-cta />
@endsection
