...
<section class="section-pad"><div class="shell"><div class="mb-10 flex items-center justify-between border-b border-line pb-5 text-[10px] uppercase tracking-[.15em]"><span>{{ $products->count() }} prendas</span><a href="{{ route('sizes') }}" class="text-muted">Guía de tallas →</a></div>@if($products->isNotEmpty())<div class="grid grid-cols-2 gap-x-3 gap-y-12 md:gap-x-6 lg:grid-cols-4 lg:gap-y-16">@foreach($products as $product)<x-product-card :product="$product" />@endforeach</div>@else<x-empty-catalog-state />@endif</div></section>
<x-catalog-cta />
@endsection
