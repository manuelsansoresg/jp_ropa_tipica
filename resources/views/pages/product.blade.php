...
<section class="pb-20 pt-24 md:pb-28 md:pt-32"><div class="shell">
    <nav class="mb-7 text-[9px] uppercase tracking-[.14em] text-muted"><a href="{{ route('collections.index') }}">Colecciones</a> / <a href="{{ route('collections.show', $product->category->slug) }}">{{ $product->category->name }}</a> / <span class="text-ink">{{ $product->name }}</span></nav>
    <div class="grid gap-10 lg:grid-cols-[1.3fr_.7fr] lg:gap-16" x-data="{ active: '{{ $product->primary_image }}' }">
        ...
        <div class="lg:sticky lg:top-32 lg:self-start">
            <p class="eyebrow">{{ $product->category->name }}</p><h1 class="display-md mt-5">{{ $product->name }}</h1>
            ...
            <p class="mt-4 text-xs text-muted">Disponibilidad sujeta a modelo, talla y color.</p>
...
@if($related->isNotEmpty())<section class="border-t border-line py-20"><div class="shell"><h2 class="display-md">También podría gustarte</h2><div class="mt-10 grid grid-cols-2 gap-4 md:grid-cols-3 md:gap-6">@foreach($related as $item)<x-product-card :product="$item" />@endforeach</div></div></section>@endif
@endsection
